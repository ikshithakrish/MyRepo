...
/******************Options  - BEGIN **************************/
	...
	public static void optionType(List<String> inputs){
		...
		System.out.println("Enter into to SeleniumAction - optionType(WebDriver, List<String>)");
		//DRIVER = driver;   //commented to test browser Launcher  ------------------
		// 0- Step Name, 1.Step Description, 2.Expected Result, 3.FindBy	4.Object (Element property) 5.Action	6.Parameter	7.Error Handler
		INPUT_PARAMS = inputs;
		System.out.println("-------"+ INPUT_PARAMS.get(4));
		
		/*--------------------------------------------------------------------
		//0-Step Name	1- Step Description	2- Expected Result	3- Object Name	4- Action	 5- Parameter	 6- Error Handler
		//String filePath = "D:\\My Assignments\\Selenium_UI_AUTOMATION\\FileHandling\\Excel\\TestAutomation.xlsx";
		String filePath = "D:\\My Assignments\\Selenium_UI_AUTOMATION\\FileHandling\\Excel\\Object Repository.xlsx";
		String sheetName = "Object Repo";
		//System.out.println("SeleniumActions------------- " +FileHandling.searchObjectRepo(filePath, sheetName, INPUT_PARAMS.get(3)) );		
		...
		if(OBJECT_NAME.contains("->")){
			OBJECT_REPO_DATA = FileHandling.searchObjectRepo(filePath, sheetName, OBJECT_NAME); // Object Name (Registration -> Email)
			// 0- Object Parameter	 1- FindByElement	 2- FindBySubElement  (ObjectRepository Index)
			LOGGER.info("Object Repository containst --- temp[0]: "+OBJECT_REPO_DATA.get(0)+ " temp[1]: "+OBJECT_REPO_DATA.get(1) + " temp[2]: "+OBJECT_REPO_DATA.get(2));
		}
		else{
			LOGGER.info("Object Not required");
		}
		---------------------------------------------------------------------- */
		// 0- Step Name 1- Step Description	2- Expected Result	3- Object Name	4- Action	5- Parameter	6- Error Handler   (TestScript Index)		
		OBJECT_NAME = INPUT_PARAMS.get(3);
		...
		case "LAUNCH_URL":
			...
			DRIVER = SeleniumActionClass.launchURL(BROWSER_INDEX, URL);
			break;
   ...
   // 0- Object Parameter	 1- FindByElement	 2- FindBySubElement  (ObjectRepository Index) - OBJECT_REPO_DATA
	public static void clickAction(){  // Applicable for submit button, Radio selection
		...
		if(isElementExist()){  // if elementExist() - BEGIN
			if(ACTION_TYPE.equalsIgnoreCase("Radio_Button")){ // if check Radio_Button - BEGIN
	...
	public static void selectOption(){
		...
		if(isElementExist()){
				...
				if(SELECT_BY_TYPE.trim().equalsIgnoreCase("selectByIndex")){	
					try{
				...
				else  if(SELECT_BY_TYPE.trim().equalsIgnoreCase("selectByValue")){
					SELECT_ELEMENT.selectByValue(VALUE);
				}
				else if(SELECT_BY_TYPE.trim().equalsIgnoreCase("selectByVisibleText")){
	...
	// make the By.? using java reflection (refer: https://stackoverflow.com/questions/45959319/java-call-a-method-with-name-stored-in-variable)
		try{  // try - locatorIdentifier(String locatorType, String locatorValue) - BEGIN
			...
			if(! ACTION_TYPE.equalsIgnoreCase("Radio_Button")){   // if not Radio_Button - BEGIN
	...
	try{  // objectRepoSearch() - try - BEGIN
		...
		if(ObjectRepo_Reader.objectRepo_Search(objectRepoParam)==null){  // null check for object (object == null)
			LOGGER.warn("object " + objectRepoParam + " not exist in objectRepository.properties file");
		}
		else{
			String objectRepoValue = ObjectRepo_Reader.objectRepo_Search(objectRepoParam);
